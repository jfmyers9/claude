...
export type NativeCompactionEntry = Omit<CompactionEntry<NativeCompactionDetails>, "details"> & {
...
function isNativeCompactionIdentity(value: unknown): value is Record<string, unknown> & NativeCompactionIdentity {
	if (!isRecord(value)) {
		return false;
	}

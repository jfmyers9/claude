...
**Address most recent review (high priority issues only):**
```bash
/address-review
```
Finds the most recent review document in .jim/notes/ and applies fixes for
high priority issues only.

**Address review by slug:**
```bash
/address-review user-authentication
```
Finds most recent review matching "user-authentication" slug and applies
high priority fixes.

**Address review by full path:**
```bash
/address-review .jim/notes/review-impl-20260201-143000-auth.md
```
Uses specific review document at provided path.
...
/address-review
```
Only addresses critical issues marked as "High" priority in review.

**High and medium priority:**
```bash
/address-review --priority=medium
```
Addresses both high and medium priority issues.

**All priorities:**
```bash
/address-review --priority=low
```
Addresses all issues (high, medium, low priority).

### Integration with Workflow

**After implementation with review:**
```bash
/implement --review
# Review identifies 5 high priority issues
/address-review
# Applies automated fixes for 3 issues, flags 2 as complex
git diff
...
# Fix remaining medium priority issues manually
...
/implement --review
/address-review --priority=medium
# Automatically fix high and medium
# Review changes
git diff
# Manually address remaining low priority issues
/commit
```
...
```bash
/next-phase --review
# Phase 2 review identifies issues
/address-review
# Automated fixes applied
/review-implementation
# Second review verifies fixes worked
/commit
```

### Common Patterns

**Pattern 1: Conservative approach (default)**
```bash
/implement --review
/address-review                    # High priority only
git diff                           # Review changes
# Manually fix remaining issues
/commit
```

**Pattern 2: Aggressive automation**
```bash
/implement --review
/address-review --priority=low     # Fix everything possible
git diff                           # Review ALL changes carefully
# Manually fix complex issues
/review-implementation             # Re-verify
/commit
```
...
# All phases done
/review-implementation
/address-review                    # Fix all phases at once
git diff
/commit
```
...
/address-review --priority=high
git diff
/review-implementation             # Check what's left
/address-review --priority=medium  # Fix next tier
git diff
/commit
```
...
Review Source: /Users/jim/workspace/claude/.jim/notes/review-impl-20260201-143000-auth.md
Fixes Summary: /Users/jim/workspace/claude/.jim/notes/fixes-20260201-143500-auth.md

Results:
  Addressed: 3 issues
  Skipped: 2 issues (complex or low priority)
  Failed: 0 issues

Files Modified: 2 files
- /Users/jim/workspace/claude/src/auth/login.ts - 2 fixes applied
- /Users/jim/workspace/claude/src/auth/validate.ts - 1 fix applied
...
1. Review changes: git diff
2. Run tests to verify fixes
3. Address 2 skipped issues manually
4. Re-run review: /review-implementation
5. Commit when ready: /commit

Fixes saved to: /Users/jim/workspace/claude/.jim/notes/fixes-20260201-143500-auth.md
```

### When to Use

**Use /address-review when:**
- Review identifies clear, straightforward issues
- You want to save time on simple fixes
- Issues have specific, actionable suggestions
- You trust the review recommendations
- You're comfortable reviewing automated changes

**Skip /address-review when:**
- All issues require architectural decisions
- Review suggestions are ambiguous or incomplete
- You prefer manual control over all changes
- Issues require understanding business logic
- Changes are high-risk or security-critical
...
```bash
/implement --review
/address-review                    # Apply review-specific fixes
/refine                            # Additional general simplification
git diff
/commit
```
...
```bash
/implement --review
# Review shows 5 high priority issues
/address-review
# 3 fixed, 2 require manual work
# Manually fix the 2 complex issues
/review-implementation
# Second review confirms all issues resolved
/commit
```

**With /save-state and /load-state:**
```bash
/implement --review
# Review identifies issues, but need to context switch
/save-state review-fixes
# ... work on something else ...
/load-state review-fixes
/address-review
/commit
```
